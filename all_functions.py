...
def run():
    ...
    if source == 1:
        ...
        try:
            ...
            print(
                "Try adding more Wallpapers to it first or change the Source from settings."
            )
    elif source == 3:
        ...
        try:
            ...
            print(
                "This feature does not guarantee you will get High quality wallpapers."
            )
...
def move_to_folder(folder_name, new_file_name, update_number=False):
    ...
    try:
        os.rename(
            f"{base_path}/Wallpaper.jpg", f"{base_path}/{folder_name}/{new_file_name}"
        )
        print(f"Picture saved as '{new_file_name}' in the '{folder_name}' folder")
        if update_number:
            data["using"]["file_name"] = "Wallpaper " + str(
                data["using"]["saved_pic_number"]
            )
        ...
        print(
            f"[ERROR] A file with the name of {new_file_name} already exists in the folder, try changing its name "
            f"first."
        )
...
def get_from_link(save_to="Wallpapers From Link"):
    ...
    if len(images) != 0:
        ...
        print(
            "Wallpapers updated! Go to Settings to if source is not already set to 'From custom link'."
        )
    else:
...
def change_quote_settings():
    ...
    if quote_randomize == 1:
        ...
        print('----------------------\nRandom is "OFF"\n----------------------')
    new_quote_settings = setting_function(
    ...
    if new_quote_settings == 1:
        ...
        update_json(
            "quote_randomize", new_quote_randomize, print_updates=False, for_quotes=True
        )
...
def quote_settings():
    while True:
        option = setting_function(
            "Quote colours",
            "Background type",
            name="quotes_settings",
            updating=False,
            auto_back_key=False,
        )
        if option == 1:
            ...
            bg_type_option = setting_function(
                "Solid colors", "Gradient colors", name="quote_bg_type", updating=False
            )
            # add to wallpaper
...
def random_gradient():
    ...
    dr = (randint(0, 255) - r) / 1000.0
    dg = (randint(0, 255) - g) / 1000.0
    db = (randint(0, 255) - b) / 1000.0
    for i in range(1920):
...
def reset():
    ...
    if confirmation == 1:
        for setting in data["using"]:
            update_json(
                setting, data["default"][setting], print_updates=False
            ) and update_json(setting, data["default"]["quote_settings"][setting])
